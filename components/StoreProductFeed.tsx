...
import { ShoppingCart, Package, Check, QrCode } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6 max-w-4xl mx-auto">
      {products.map((p) => {
        ...
        return (
          <Link key={p.id} href={`/s/${slug}/products/${p.id}`}>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden hover:shadow-lg hover:border-red-200 dark:hover:border-red-900 transition-all cursor-pointer group"
            >
              <div className="aspect-[4/3] bg-zinc-100 dark:bg-zinc-800 relative overflow-hidden">
                ...
                {p.barcode && (
                  <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 bg-black/60 text-white text-[9px] font-semibold rounded-lg backdrop-blur-sm">
              ...
              <div className="p-4 md:p-5 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1 flex-1 min-w-0">
                    <h3 className="text-sm md:text-base font-bold text-zinc-900 dark:text-zinc-100 leading-tight truncate group-hover:text-red-600 transition-colors">{p.name}</h3>
                    {p.desc && (
