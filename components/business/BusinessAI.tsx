...
export default function BusinessAI({ agentName, store, products, setProducts, customers, setCustomers, orders, setOrders, services, setServices, totalSales, kbEntries, setKbEntries, campaigns, setCampaigns, automations, setAutomations, onPersist, onExecuteAutomations, onSaveStore, maxMessages = 999, planLimits }: {
  ...
  {"type":"navigateToSection","section":"appointments"}`,
      ...
      const navigationOverride = /(?:lleva|lle|va|vamos|dirige|manda|navega|abre|muestra|enseña|ir\s*a|ir\s*al|abrir|redirige|vete|quiero\s+ir|ponme|ponte|accede|entra|dashboard|panel|inicio)/i.test(msg)
        ? "\n\n⚠️ INSTRUCCIÓN CRÍTICA: El usuario ACABA de pedirte que lo lleves a una sección del panel. DEBES incluir el JSON navigateToSection en tu respuesta. Ignora cualquier historial previo donde ya hayas navegado. Esta es una orden directa del usuario.\n\n"
        : "";

      const defaultSystem = `${navigationOverride}${catPrompt} Ayudas al usuario a administrar su negocio "${agentName}" dentro de Jandosoft.
...
- **NAVEGAR = ACCIÓN INMEDIATA**: Si el usuario te pide ir a una sección del panel (dashboard, productos, clientes, citas, etc.), genera el JSON navigateToSection INMEDIATAMENTE. Incluye SIEMPRE el bloque JSON aunque ya hayas navegado antes. El usuario puede pedir navegar a la misma sección varias veces y DEBES navegarlo cada vez. Secciones válidas: dashboard, products, services, customers, orders, analytics, team, orgsettings, appointments, invoices, campaigns, integrations, ai, knowledgebase, agentconfig, agentinstall, smartforms, automations, menu, recipes, courses, classes, students, grades, clients, case_files, hearings, documents, medical_records, prescriptions, doctors, inventory, gallery, testimonials, barbers, queue, barber_history, restaurant, reservations, promotions, loyalty, restaurant_reviews, waiter_calls, floor_plan, restaurant_orders.
${activeDescriptions}
- IMPORTANTE: SIEMPRE que el usuario te pida crear, modificar, eliminar, enviar o navegar a algo, DEBES incluir el bloque JSON con las acciones correspondientes. No te limites a decir "lo haré" sin generar el JSON.
