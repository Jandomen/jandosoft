...
export default function TeamPanel() {
  ...
  const loadMembers = async () => {
    try {
      ...
      showToast(t("team.error_load"), "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const addMember = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { showToast(data.error || t("team.error_add"), "error"); return; }
      setMembers(data.members || []);
      setInviteEmail("");
      setShowInvite(false);
      showToast(t("team.added"), "success");
    } catch {
      showToast(t("team.error_connection"), "error");
    } finally {
      setAdding(false);
    }
  };
  ...
  const removeMember = async (memberId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { showToast(data.error || t("team.error_delete"), "error"); return; }
      setMembers(data.members || []);
      showToast(t("team.deleted"), "success");
    } catch {
      showToast(t("team.error_connection"), "error");
    }
  };
  ...
  const changeRole = async (memberId: string, newRole: "admin" | "member") => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { showToast(data.error || t("team.error_role"), "error"); return; }
      setMembers(data.members || []);
      showToast(t("team.role_updated"), "success");
    } catch {
      showToast(t("team.error_connection"), "error");
    }
  };
  ...
  if (loading) {
    return (
      <div className="space-y-8">
        <h3 className="text-2xl max-[400px]:text-xl font-black italic text-zinc-950 uppercase tracking-tighter"><Users className="w-6 h-6 max-[400px]:w-5 max-[400px]:h-5 inline mr-3 text-red-600" />{t("team.title")}</h3>
    ...
    <div className="space-y-8 max-[400px]:space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4 max-[400px]:gap-3">
        <h3 className="text-2xl max-[400px]:text-xl font-black italic text-zinc-950 uppercase tracking-tighter">
          <Users className="w-6 h-6 max-[400px]:w-5 max-[400px]:h-5 inline mr-3 text-red-600" />{t("team.title")}
        ...
        <motion.button whileTap={{ scale: 0.95 }} onClick={() => setShowInvite(!showInvite)} className="px-6 max-[400px]:px-5 py-3 max-[400px]:py-2.5 bg-red-600 text-white rounded-2xl font-black text-xs max-[400px]:text-[10px] italic hover:bg-red-700 transition-all shadow-xl flex items-center gap-2">
          <UserPlus className="w-4 h-4 max-[400px]:w-3.5 max-[400px]:h-3.5" /> {t("team.invite")}
        </motion.button>
      </div>

      {showInvite && (
        <div className="bg-zinc-50 p-6 max-[400px]:p-5 rounded-[2rem] border border-zinc-100 space-y-4">
          <p className="text-xs max-[400px]:text-[11px] font-black italic text-zinc-950 uppercase tracking-tighter">{t("team.new_member")}</p>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="flex-1 relative">
              <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
              <input type="email" placeholder={t("team.email_placeholder")} value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} className="w-full bg-white pl-12 pr-4 py-4 max-[400px]:py-3.5 rounded-xl border border-zinc-200 outline-none font-medium focus:border-red-200 transition-all text-sm" />
            </div>
      ...
      <div className="space-y-2">
        {members.map((m, i) => (
          <div key={m._id || m.email || i} className="flex items-center justify-between p-4 max-[400px]:p-3 bg-zinc-50 rounded-2xl border border-zinc-100 hover:border-red-200 transition-all group">
            <div className="flex items-center gap-4 max-[400px]:gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 max-[400px]:gap-1.5">
                  <p className="font-black italic text-zinc-950 text-sm truncate">{m.name || t("team.no_name")}</p>
            ...
            <div className="flex items-center gap-2 max-[400px]:gap-1.5 shrink-0">
                ...
                <>
                  <div className="relative group/role">
                    <motion.button whileTap={{ scale: 0.95 }} className="px-3 max-[400px]:px-2.5 py-1.5 bg-white rounded-lg border border-zinc-200 text-[9px] max-[400px]:text-[8px] font-black italic flex items-center gap-1 hover:border-red-200 transition-all uppercase">
                      <UserCog className="w-3 h-3 max-[400px]:w-2.5 max-[400px]:h-2.5" /> {m.role === "admin" ? t("team.role_admin") : t("team.role_member")}
                    ...
                    <div className="absolute right-0 top-full mt-1 bg-white rounded-xl border border-zinc-100 shadow-lg py-1 min-w-[120px] hidden group-hover/role:block z-10">
                      <button onClick={() => changeRole(m._id, "admin")} className="w-full px-4 py-2 text-[10px] font-bold italic text-left hover:bg-zinc-50 flex items-center gap-2"><Shield className="w-3 h-3 text-red-500" /> {t("team.role_admin")}</button>
                      <button onClick={() => changeRole(m._id, "member")} className="w-full px-4 py-2 text-[10px] font-bold italic text-left hover:bg-zinc-50 flex items-center gap-2"><Users className="w-3 h-3 text-zinc-500" /> {t("team.role_member")}</button>
                    </div>
                  </div>
              ...
              {m.role === "owner" && (
                <span className="px-3 max-[400px]:px-2.5 py-1.5 bg-amber-50 text-amber-700 rounded-lg text-[9px] max-[400px]:text-[8px] font-black italic uppercase shrink-0">{t("team.role_owner")}</span>
              )}
            </div>
          </div>
        ))}
