...
export default function AnalyticsPanel({ storeId }: { storeId: string | number }) {
  ...
  if (loading) {
    return (
      <div className="space-y-8">
        <h3 className="text-2xl max-[400px]:text-xl font-black italic text-zinc-950 uppercase tracking-tighter">Analytics</h3>
  ...
  if (!data || !data.dailyBreakdown) {
    return (
      <div className="space-y-8">
        <h3 className="text-2xl max-[400px]:text-xl font-black italic text-zinc-950 uppercase tracking-tighter"><TrendingUp className="w-6 h-6 max-[400px]:w-5 max-[400px]:h-5 inline mr-3 text-red-600" />Analytics</h3>
        <div className="bg-zinc-50 p-10 max-[400px]:p-8 p-12 rounded-[2.5rem] border border-zinc-100 text-center space-y-3">
          ...
          <p className="text-sm max-[400px]:text-xs font-black italic text-zinc-300 uppercase tracking-wider">Sin datos aún</p>
          <p className="text-[10px] font-bold text-zinc-300 italic">Comparte tu tienda pública para empezar a recibir visitas.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-[400px]:gap-3 gap-5">
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Vistas totales</p>
        ...
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Visitantes únicos</p>
        ...
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Vistas hoy</p>
        ...
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Promedio diario</p>
      ...
      <div className="bg-white p-6 max-[400px]:p-5 p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm">
        <h4 className="text-sm max-[400px]:text-xs font-black italic text-zinc-950 uppercase tracking-tighter mb-4 max-[400px]:mb-4 mb-6">Vistas por día</h4>
        {data.dailyBreakdown.length === 0 ? (
          <div className="py-12 text-center">
            <BarChart3 className="w-10 h-10 text-zinc-200 mx-auto mb-3" />
            <p className="text-sm font-black italic text-zinc-300 uppercase tracking-wider">Sin datos aún</p>
            <p className="text-[10px] text-zinc-200 font-bold italic mt-1">Comparte tu tienda pública para empezar a recibir visitas.</p>
          </div>
        ) : (
      ...
      <div className="bg-white p-6 max-[400px]:p-5 p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm">
        <h4 className="text-sm max-[400px]:text-xs font-black italic text-zinc-950 uppercase tracking-tighter mb-4 max-[400px]:mb-4 mb-6">Páginas más visitadas</h4>
        {!data.topPages || data.topPages.length === 0 ? (
          <p className="text-xs font-bold italic text-zinc-300">Sin datos</p>
        ) : (
          <div className="space-y-2">
            {data.topPages.map((page, i) => {
              const label = page._id === "/" ? "Inicio" : page._id === "/products" ? "Productos" : page._id === "/services" ? "Servicios" : page._id === "/contact" ? "Contacto" : page._id;
                ...
                <div key={page._id} className="flex items-center gap-4 p-3 bg-zinc-50 rounded-xl hover:bg-zinc-100 transition-all">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] font-black text-zinc-400">{page.views} vistas</span>
                    </div>
