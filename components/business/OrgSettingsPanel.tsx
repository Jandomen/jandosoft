...
export default function OrgSettingsPanel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        showToast(t("org.toast_load_error"), "error");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const saveOrg = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { showToast(data.error || t("org.toast_save_error"), "error"); return; }
      setOrg(data.organization);
      showToast(t("org.toast_updated"), "success");
    } catch {
      showToast(t("org.toast_connection_error"), "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="space-y-8">
        <h3 className="text-2xl max-[400px]:text-xl font-black italic text-zinc-950 uppercase tracking-tighter"><Settings className="w-6 h-6 max-[400px]:w-5 max-[400px]:h-5 inline mr-3 text-red-600" />{t("org.title")}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-[400px]:gap-5 gap-6">
        <div className="bg-white p-6 max-[400px]:p-6 p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm space-y-5 max-[400px]:space-y-5 space-y-6">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm max-[400px]:text-xs font-black italic text-zinc-950 uppercase tracking-tighter">{t("org.organization")}</p>
          </div>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-zinc-400 uppercase italic ml-1 tracking-widest">{t("org.name")}</label>
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={saveOrg} disabled={!orgName.trim() || saving} className="px-7 max-[400px]:px-7 px-8 py-3.5 max-[400px]:py-3.5 py-4 bg-red-600 text-white rounded-2xl font-black text-xs italic hover:bg-red-700 transition-all shadow-xl disabled:opacity-50 flex items-center gap-2">
              <Save className="w-3.5 h-3.5 max-[400px]:w-3.5 max-[400px]:h-3.5 w-4 h-4" /> {saving ? t("org.saving") : t("org.save")}
            </motion.button>
          </div>
        </div>

        <div className="bg-white p-6 max-[400px]:p-6 p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm space-y-5 max-[400px]:space-y-5 space-y-6">
          <p className="text-sm max-[400px]:text-xs font-black italic text-zinc-950 uppercase tracking-tighter">{t("org.details")}</p>
          <div className="space-y-3 max-[400px]:space-y-3 space-y-4">
            <div className="flex items-center justify-between p-3.5 max-[400px]:p-3.5 p-4 bg-zinc-50 rounded-xl">
              <span className="text-[9px] max-[400px]:text-[9px] text-[10px] font-bold text-zinc-400 italic uppercase flex items-center gap-2"><Hash className="w-2.5 h-2.5 max-[400px]:w-2.5 max-[400px]:h-2.5 w-3 h-3" /> {t("org.slug")}</span>
              <span className="text-[11px] max-[400px]:text-[11px] text-xs font-black italic text-zinc-950 truncate ml-2">{org?.slug}</span>
            </div>
            <div className="flex items-center justify-between p-3.5 max-[400px]:p-3.5 p-4 bg-zinc-50 rounded-xl">
              <span className="text-[9px] max-[400px]:text-[9px] text-[10px] font-bold text-zinc-400 italic uppercase flex items-center gap-2"><Calendar className="w-2.5 h-2.5 max-[400px]:w-2.5 max-[400px]:h-2.5 w-3 h-3" /> {t("org.created")}</span>
              <span className="text-[11px] max-[400px]:text-[11px] text-xs font-black italic text-zinc-950 text-right ml-2">{org?.createdAt ? new Date(org.createdAt).toLocaleDateString(t("_locale") || "es", { year: "numeric", month: "long", day: "numeric" }) : "-"}</span>
            </div>
          </div>
        </div>
      </div>
