...
export default function CampaignsPanel({ campaigns, setCampaigns, onPersist }: CampaignsPanelProps) {
  ...
  const save = () => {
    ...
    setShowForm(false);
    setEditingCampaign(null);
    showToast(editingCampaign ? "Campaña actualizada" : "Campaña creada", "success");
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 max-[400px]:gap-3 gap-4">
        <h3 className="text-2xl max-[400px]:text-xl font-black italic text-zinc-950 uppercase tracking-tighter">
          <Megaphone className="w-6 h-6 max-[400px]:w-5 max-[400px]:h-5 inline mr-3 text-red-600" />Campañas
        ...
        <motion.button whileTap={{ scale: 0.95 }} onClick={openNew} disabled={campaigns.length >= 50} className="px-5 max-[400px]:px-5 px-6 py-2.5 max-[400px]:py-2.5 py-3 bg-red-600 text-white rounded-2xl font-black text-[10px] max-[400px]:text-[10px] text-xs italic hover:bg-red-700 transition-all shadow-xl flex items-center gap-2 disabled:opacity-50">
          <Plus className="w-3.5 h-3.5 max-[400px]:w-3.5 max-[400px]:h-3.5 w-4 h-4" /> NUEVA CAMPAÑA
        </motion.button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-[400px]:gap-3 gap-5">
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Enviados</p>
        ...
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Abiertos</p>
        ...
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Clicks</p>
        ...
        <div className="bg-white p-5 max-[400px]:p-5 p-6 rounded-[2rem] border border-zinc-100 shadow-sm space-y-2 max-[400px]:space-y-2 space-y-3">
          ...
          <p className="text-[8px] max-[400px]:text-[8px] text-[9px] font-black text-zinc-400 uppercase italic">Tasa apertura</p>
      ...
      {campaigns.length === 0 ? (
        <div className="py-20 text-center italic font-black uppercase tracking-widest text-zinc-200">
          No hay campañas aún. Crea tu primera campaña de marketing.
        </div>
      ) : (
              ...
              <div className="flex items-center gap-3 max-[400px]:gap-3 gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 max-[400px]:gap-1.5 gap-2 mt-0.5 max-[400px]:mt-0.5 mt-1 flex-wrap">
                    ...
                    <span className="text-[8px] max-[400px]:text-[8px] text-[9px] text-zinc-400 font-bold italic">{c.type === "email" ? "Email" : "SMS"}</span>
                    {c.scheduledAt && (
              ...
              <div className="flex items-center gap-1.5 max-[400px]:gap-1.5 gap-2 shrink-0">
                ...
                {c.status === "sent" && c.stats.sent > 0 && (
                  <div className="hidden max-[400px]:hidden md:flex items-center gap-2 mr-2 text-[7px] max-[400px]:text-[7px] text-[8px] font-bold italic text-zinc-400">
                    <span title="Enviados"><Send className="w-2 h-2 max-[400px]:w-2 max-[400px]:h-2 w-3 h-3 inline mr-0.5" />{c.stats.sent}</span>
                    <span title="Abiertos"><Eye className="w-2 h-2 max-[400px]:w-2 max-[400px]:h-2 w-3 h-3 inline mr-0.5" />{c.stats.opened}</span>
                    <span title="Clicks"><MousePointerClick className="w-2 h-2 max-[400px]:w-2 max-[400px]:h-2 w-3 h-3 inline mr-0.5" />{c.stats.clicked}</span>
                  </div>
                )}
                ...
                {c.status === "draft" && (
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => sendCampaign(c.id)} disabled={sending === c.id} className="px-3 max-[400px]:px-3 px-4 py-1.5 max-[400px]:py-1.5 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[8px] max-[400px]:text-[8px] text-[9px] font-black italic hover:bg-emerald-100 transition-all flex items-center gap-1 border border-emerald-100">
                    {sending === c.id ? <Loader2 className="w-2.5 h-2.5 max-[400px]:w-2.5 max-[400px]:h-2.5 w-3 h-3 animate-spin" /> : <Send className="w-2.5 h-2.5 max-[400px]:w-2.5 max-[400px]:h-2.5 w-3 h-3" />} ENVIAR
                  </motion.button>
                )}
                {c.status === "scheduled" && (
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => togglePause(c.id)} className="px-2.5 max-[400px]:px-2.5 px-3 py-1.5 max-[400px]:py-1.5 py-2 bg-zinc-50 text-zinc-500 rounded-xl text-[8px] max-[400px]:text-[8px] text-[9px] font-black italic hover:bg-zinc-100 transition-all flex items-center gap-1 border border-zinc-100">
                    <Pause className="w-2.5 h-2.5 max-[400px]:w-2.5 max-[400px]:h-2.5 w-3 h-3" /> PAUSAR
                  </motion.button>
                )}
                {c.status === "paused" && (
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => togglePause(c.id)} className="px-2.5 max-[400px]:px-2.5 px-3 py-1.5 max-[400px]:py-1.5 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[8px] max-[400px]:text-[8px] text-[9px] font-black italic hover:bg-emerald-100 transition-all flex items-center gap-1 border border-emerald-100">
                    <Play className="w-2.5 h-2.5 max-[400px]:w-2.5 max-[400px]:h-2.5 w-3 h-3" /> REANUDAR
                  </motion.button>
                )}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/40 backdrop-blur-md flex items-center justify-center p-6" onClick={() => { setShowForm(false); setEditingCampaign(null); }}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="w-full max-w-lg bg-white rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 shadow-4xl relative" onClick={e => e.stopPropagation()}>
              ...
              <h3 className="text-xl md:text-2xl font-black italic text-zinc-950 mb-4 md:mb-6 uppercase tracking-tighter">{editingCampaign ? "Editar Campaña" : "Nueva Campaña"}</h3>
              <div className="space-y-3 md:space-y-5 max-h-[70vh] overflow-y-auto pr-1 md:pr-2">
                <div>
                  <label className="text-[9px] font-black text-zinc-400 uppercase italic ml-1 tracking-widest">Nombre de la campaña</label>
                  <input type="text" placeholder="Ej. Ofertas de verano" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full bg-zinc-50 p-3 md:p-4 rounded-xl border border-zinc-100 outline-none font-medium focus:bg-white focus:border-red-200 transition-all mt-1 text-sm" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[9px] font-black text-zinc-400 uppercase italic ml-1 tracking-widest">Audiencia</label>
                ...
                <div>
                  <label className="text-[9px] font-black text-zinc-400 uppercase italic ml-1 tracking-widest">Asunto / Título</label>
                  <input type="text" placeholder={form.type === "email" ? "Ej. No te pierdas nuestras ofertas" : "Ej. Oferta especial para ti"} value={form.subject} onChange={e => setForm({...form, subject: e.target.value})} className="w-full bg-zinc-50 p-3 md:p-4 rounded-xl border border-zinc-100 outline-none font-medium focus:bg-white focus:border-red-200 transition-all mt-1 text-sm" />
                </div>

                <div>
                  <label className="text-[9px] font-black text-zinc-400 uppercase italic ml-1 tracking-widest">Mensaje</label>
                  <textarea placeholder="Escribe el contenido de tu campaña..." value={form.body} onChange={e => setForm({...form, body: e.target.value})} className="w-full bg-zinc-50 p-3 md:p-4 rounded-xl border border-zinc-100 outline-none font-medium focus:bg-white focus:border-red-200 transition-all resize-none h-32 mt-1 text-sm" />
                  <p className="text-[8px] font-bold text-zinc-400 italic mt-1 ml-1">{form.body.length} caracteres{form.type === "sms" ? ` / 160 (${Math.ceil(form.body.length / 160)} SMS)` : ""}</p>
                </div>

                <div>
                  <label className="text-[9px] font-black text-zinc-400 uppercase italic ml-1 tracking-widest">Programar envío</label>
                  <div className="flex gap-3 mt-1">
                    <button onClick={() => setForm({...form, schedule: "now"})} className={cn("flex-1 py-3.5 rounded-xl font-black text-xs italic transition-all border", form.schedule === "now" ? "bg-red-600 text-white border-red-600 shadow-md" : "bg-zinc-50 text-zinc-500 border-zinc-100 hover:border-zinc-200")}>
                      <Send className="w-4 h-4 inline mr-1.5" />Ahora
                    </button>
                    <button onClick={() => setForm({...form, schedule: "scheduled"})} className={cn("flex-1 py-3.5 rounded-xl font-black text-xs italic transition-all border", form.schedule === "scheduled" ? "bg-red-600 text-white border-red-600 shadow-md" : "bg-zinc-50 text-zinc-500 border-zinc-100 hover:border-zinc-200")}>
                      <Clock className="w-4 h-4 inline mr-1.5" />Programar
                    </button>
                  </div>
                ...
                <button onClick={save} disabled={!form.name || !form.subject || !form.body || (form.schedule === "scheduled" && !form.scheduledAt)} className="w-full py-4 md:py-5 bg-red-600 text-white rounded-2xl font-black italic text-sm md:text-base hover:bg-red-700 transition-all shadow-xl disabled:opacity-50 flex items-center justify-center gap-2">
                  {editingCampaign ? "ACTUALIZAR CAMPAÑA" : form.schedule === "scheduled" ? "PROGRAMAR CAMPAÑA" : "GUARDAR BORRADOR"}
                </button>
              </div>
            </motion.div>
          </motion.div>
